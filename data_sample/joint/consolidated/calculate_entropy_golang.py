...
r_consolidated_params_json = 'consolidated_gauss_params.json'
w_consolidated_percent_json = 'consolidated_entropysum_percent.json'

def write_dict_to_json(dict, json_to_write):
    df_params = pd.DataFrame.from_dict({k:[v] for k,v in dict.items()},orient="index",columns=["user profit percentage"])
...
#   print("before_extension_half",)
  ...
  for user_str in users_list:
    ...
    for subkey in data[user_str].keys():
        this_d_data = data[user_str][subkey][list(data[user_str][subkey].keys())[0]]
        ...
        joint_entropy.append(I[0])  
    print("joint_entropy",joint_entropy) 
    user_entropies_dict[user_str] = joint_entropy         
    user_entrosum_dict[user_str] = sum(joint_entropy)   
    total_entrosum += sum(joint_entropy)   
