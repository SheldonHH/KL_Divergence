...
def read_from_csv(r_dense_trimmed_data_path1,each_col_freq_path):
    df = pd.read_csv(r_dense_trimmed_data_path1)
    write_to_json(df,each_col_freq_path)


def write_to_json(df,each_col_freq_path):
    json_file_to_write = open(each_col_freq_path, 'w')
...
def main():
    ...
    r_dense_trimmed_data_path1 = first_half+"trimmed/dense_trimmed_"+before_extension_half+".csv"
    each_col_freq_path = first_half + "independent/each_col_freq_"+before_extension_half+".json"
    read_from_csv(r_dense_trimmed_data_path1,each_col_freq_path)
