...
def freq_to_gauss(true_datapath,  inputfile,  col_counter, raw_data_size, username, initial_counter, call_sign_folder):
...
def f_to_g(Dir, username, counter_initial, call_sign_folder):
    ...
    for feat_counter in range(4096):
        ...
        freq_to_gauss(Dir, inputfile, str(
            feat_counter), str(rds), username, counter_initial, call_sign_folder)
...
def main():
    ...
    gauss_folder_ID = "two_gauss"
    num_gauss = 2
    for uname in Inputdicts.keys():
        f_to_g(Dir, uname, num_gauss, gauss_folder_ID)
