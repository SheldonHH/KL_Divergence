...
def main():
    ...
    os.chdir(dir_str)   
    ## create subportion for each file
    ## sample subportion and generation gmm
    file_gauss_dict = {}
    for root,dirs,files in os.walk(rounded_data_directory):
        for file in files:
            dimension_min_with_params = {}
            if file.endswith(".csv") and file.startswith("rounded"):
                f=open(file, 'r')
                print(file[findNth(file,"_",1)+1:findNth(file,"_",3)])
