...
import json
import csv
from consolidate_each_gauss import *
from g_to_e import *
import math
...
def raw_data_size(username):
    numpy_array = np.loadtxt(
        "/home/code/entropy/user_gauss_params/data/q/"+username+"/"+username+"_0_q.csv", delimiter=",")
...
# Generate Frequency of Datasets
def create_freq(Inputdicts):
    t0 = time.time()
    df = pd.read_csv("/home/code/entropy/user_gauss_params/data/combine/preprocessed_dataset.csv", delimiter=" ")
    t1 = time.time()
    ...
    for userkey, value in Inputdicts.items():
        each_fea = []
        uq_path = "/home/code/entropy/user_gauss_params/data/q/"+userkey+"/"
    ...
    t1 = time.time()
    print("create_freq duration:",t1-t0)


def ga_entropy(Inputdicts):
    Dir = "/home/code/entropy/user_gauss_params/data/"
    sample_percent = 20
    
    ## for static test only
