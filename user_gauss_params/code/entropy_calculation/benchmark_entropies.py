...
def benchmark(Inputdicts, location_of_combined_file, rawdata_dir):
    ...
    xi_list = [] 
    for key, value in dict(sorted(Inputdicts.items())).items():
        ...
        dfRange.to_csv("/home/code/entropy/user_gauss_params/data/combine/" +
                    key+"_combined.csv", header=False, index=False)
    t1 = time.time()
    print("Step 1. Generate each individual user file: ",t1-t0)


    # 2.1 max and min for each file
    t0 = time.time()
    combined_mxin = pd.concat(xi_list)
    dfTotalXI = combined_mxin.agg([min, max])
    dfTotalXI.to_csv("/home/code/entropy/user_gauss_params/data/combine/dfTotalXI.csv", header=False, index=False)
    t1 = time.time()
    ...
    t0 = time.time()
    ui_row_list = []    
    for key, value in Inputdicts.items():
    ...
    np.savetxt(rawdata_dir+"/results/bench_percent_list.csv", bench_percent_list, delimiter=",")
    print(bench_percent_list)


def be_entropy(Inputdicts):
    combined_file_name =  "preprocessed_dataset.csv" # dataset from NIST
    rawdata_dir = "/home/code/entropy/user_gauss_params/data/combine/"
   
    t0 = time.time()
    removeAllFiles(rawdata_dir+"sums/")
    removeAllFiles(rawdata_dir+"results/")
    t1 = time.time()

    print("Removed All Files sum Folders: ", t1-t0)
    benchmark(Inputdicts, rawdata_dir+combined_file_name, rawdata_dir)
